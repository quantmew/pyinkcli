...
当前 hooks 状态建立在全局 `_current_state` 上，不具备组件级实例隔离能力。[state.py](/mnt/hdd1/ink-python/src/pyinkcli/hooks/state.py#L27)

### Scope

- 重构 `src/pyinkcli/hooks/state.py`
...
- [state.py](/mnt/hdd1/ink-python/src/pyinkcli/hooks/state.py)
- [reconciler.py](/mnt/hdd1/ink-python/src/pyinkcli/reconciler.py)
...
当前 `update_container()` 每次都会先删光 children 再重新建树。[reconciler.py](/mnt/hdd1/ink-python/src/pyinkcli/reconciler.py#L75)
...
- [reconciler.py](/mnt/hdd1/ink-python/src/pyinkcli/reconciler.py)
- [dom.py](/mnt/hdd1/ink-python/src/pyinkcli/dom.py)
...
Python `render()` 直接返回 `Ink` 实例，缺少 JS `Instance` 语义中的 `waitUntilRenderFlush`、`cleanup` 等能力。[ink.py](/mnt/hdd1/ink-python/src/pyinkcli/ink.py#L683) [render.ts](/mnt/hdd1/ink-python/js_source/ink/src/render.ts#L138)
...
- [ink.py](/mnt/hdd1/ink-python/src/pyinkcli/ink.py)
- [__init__.py](/mnt/hdd1/ink-python/src/pyinkcli/__init__.py)
...
当前输入解析只覆盖简化 ANSI 序列，没有 JS 那套 `input-parser` 的 chunk 级处理能力。[use_input.py](/mnt/hdd1/ink-python/src/pyinkcli/hooks/use_input.py#L119)
...
- 新增 `src/pyinkcli/input_parser.py`
- [use_input.py](/mnt/hdd1/ink-python/src/pyinkcli/hooks/use_input.py)
...
- [use_stdin.py](/mnt/hdd1/ink-python/src/pyinkcli/hooks/use_stdin.py)
- [components/app.py](/mnt/hdd1/ink-python/src/pyinkcli/components/app.py)
...
Python `log_update` 只有表面参数，没有 JS 那种标准模式和增量模式的核心差异。[log_update.py](/mnt/hdd1/ink-python/src/pyinkcli/log_update.py#L20)
...
- [log_update.py](/mnt/hdd1/ink-python/src/pyinkcli/log_update.py)
- [ink.py](/mnt/hdd1/ink-python/src/pyinkcli/ink.py)
...
当前 `Output` 只是二维字符串网格，对 ANSI 和复杂裁剪场景支持不足。[output.py](/mnt/hdd1/ink-python/src/pyinkcli/output.py#L181)
...
Python `App` 目前只是简化包裹层，无法承担 JS `App` 的 runtime 协调职责。[app.py](/mnt/hdd1/ink-python/src/pyinkcli/components/app.py#L20)
...
- [use_paste.py](/mnt/hdd1/ink-python/src/pyinkcli/hooks/use_paste.py)
- [use_stdin.py](/mnt/hdd1/ink-python/src/pyinkcli/hooks/use_stdin.py)
...
Python renderer 虽有 screen reader 分支，但 hook 层未打通，当前 `use_is_screen_reader_enabled()` 直接返回 `False`。[use_is_screen_reader_enabled.py](/mnt/hdd1/ink-python/src/pyinkcli/hooks/use_is_screen_reader_enabled.py#L10)
...
Python 只有 `kitty_keyboard` 选项字段，没有真正实现链路。[ink.py](/mnt/hdd1/ink-python/src/pyinkcli/ink.py#L69)
